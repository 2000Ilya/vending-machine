import React from "react";

const BanknoteIcon = () => {
  return (
    <svg
      version="1.1"
      id="Layer_1"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 511.88 511.88"
    >
      <rect
        y="117.3"
        style={{ fill: "#8CC153" }}
        width="511.88"
        height="277.28"
      />
      <path
        style={{ fill: "#A0D468" }}
        d="M479.89,330.594V181.285c-17.668,0-31.992-14.325-31.992-31.993H63.922
	c0.047,0.671,0.062,1.328,0.062,2c0,17.668-14.325,31.993-31.992,31.993v149.31c16.996,0,30.898,13.248,31.93,29.992h383.976
	C447.898,344.92,462.222,330.594,479.89,330.594z"
      />
      <path
        style={{ fill: "#8CC153" }}
        d="M319.927,255.94c0,35.336-28.65,63.986-63.986,63.986s-63.985-28.65-63.985-63.986
	c0-35.335,28.649-63.985,63.985-63.985S319.927,220.605,319.927,255.94z"
      />
    </svg>
  );
};

export default BanknoteIcon;
